...
//			HAL_GPIO_WritePin(THAPCOI_GPIO_Port,THAPCOI_Pin,GPIO_PIN_SET);
		...
		if(vRS485.RxFlag == 1){   
			...
			vRS485.txBuff[0] = 0xFF;
			vRS485.txBuff[1] = 0xFE; 
			vRS485.txBuff[2] = (address_chute>>8)&(0xFF);
			vRS485.txBuff[3] = address_chute&(0xFF);
			vRS485.txBuff[4] = 0x00; // so luong san pham
			vRS485.txBuff[5] = led_status; // trang thai mang
...
//			vRS485.RxFlag = 0;   
//			
//			hour = vRS485.rxBuff[1];
//			minute = vRS485.rxBuff[2];
//			dagui = (vRS485.rxBuff[3] << 16) | (vRS485.rxBuff[4] << 8 ) | vRS485.rxBuff[5];
//			danhan = (vRS485.rxBuff[6] << 16) | (vRS485.rxBuff[7] << 8 ) | vRS485.rxBuff[8];
//			miss_mess = (vRS485.rxBuff[9] << 8 ) | vRS485.rxBuff[10];
//			
//			/*Xu ly data nhan duoc o day*/
//			//flag_display_lcd = 1;
//			num_rx++;
...
//		}
		...
		if(flag_display_lcd >= 1000){
			...
			sprintf(ROW0, "ID: %u", address_chute);
			sprintf(ROW1, "So luong: %u", rxbuff[5]);			
			sprintf(ROW2, "%s", chute_status);
			sprintf(ROW3, "%u", vRS485.cnt_mess_received);
//			sprintf(ROW0, "ID: %d h %d m", hour, minute);
